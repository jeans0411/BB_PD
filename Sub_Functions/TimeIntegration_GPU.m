...
tic;
fail=gpuArray.ones(Totalbonds,1,'logical');                      
Stretch=gpuArray.zeros(Totalbonds,1,'single');
% DeformedLength=zeros(Totalbonds,1);
disp=gpuArray.zeros(Totalnodes,Nod,'single');                         % Displacement of a material point
disp_forward=gpuArray.zeros(Totalnodes,Nod,'single');
v=gpuArray.zeros(Totalnodes,Nod,'single');                            % Velocity of a material point
v_forward=gpuArray.zeros(Totalnodes,Nod,'single');
a=gpuArray.zeros(Totalnodes,Nod,'single');                            % Acceleration of a material point
Displacement_node=gpuArray.zeros(nt,1,'single');                      % History of displacement at selected node
% ForceHistory=zeros(nt,1);                           % History of applied force
ReactionForce=gpuArray.zeros(nt,Nod,'single');                          
LOAD_step=gpuArray.zeros(nt,Nod,'single');
nt0=sum([1:nt].^0.5);
failnum=gpuArray.zeros(nt,1,'single');
dens=gpuArray(single([dens dens dens]));
counter=0;tt0=0;
coordinates=gpuArray(single(coordinates));
...
for tt=countmin:nt
    ...
    Nforce=gpuArray.zeros(Totalnodes,Nod,'single');  % Nodal force - initialise for every time step
    DisplacedCoordinates=coordinates+disp;  %gpu
    nt1=sum((nt-[0:tt-1]).^0.5);
    loadfactor=0.5*(1-cos(pi*nt1/nt0));
    bodyforce2=gpuArray(single(bodyforce*loadfactor));
...
if mod(round(perc_progress,5),10)==0
    save([num2str(tt0),'.mat']); % Save workspace to local computer every 2500 time steps
end
